    ...
    .then((responseData) => {
      ...
      if (responseData.token === "201") {
        ...
        if (responseData.message === "Signup Successful") {
          ...
          // Store login status for immediate access
  ...
  fetch(url, {
    ...
    .then((responseData) => {
      ...
      if (responseData.token) {
        // Store the token securely (e.g., in cookies or local storage)
        localStorage.setItem("token", responseData.token);
        window.location.href = "../pages/about.html"; // Redirect to about page if login is successful
      } else {
        ...
        setTimeout(() => {
          errorMessageElement.textContent = "";
        }, 60000); // Clear after 5 seconds (adjust as needed)
      }
    })
    .catch((error) => {
      console.error("Error:", error);
